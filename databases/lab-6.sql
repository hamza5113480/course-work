...
--> Write a query to list the employees in dept 10 with the same job as anyone in the ‘CSE’ dept.
SELECT * FROM EMPLOYEE
WHERE DEPTNO = 10
  AND JOB IN (SELECT JOB FROM EMPLOYEE WHERE DEPTNO = (SELECT DEPTNO FROM DEPARTMENT WHERE DNAME = 'CSE'));

--> Write a query to create a view, which list the employees including dept name.
